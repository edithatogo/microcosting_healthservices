...
- **No NEP26 compiled/Python reference files from IHACPA.** IHACPA distributes NWAU calculators only as SAS `.7Z` archives and `.xlsb` Excel workbooks. No standalone Python, R, or compiled reference implementation is published. This means any Python reimplementation (e.g. a verified Python NWAU calculator) must be validated directly against the SAS and/or Excel outputs — there is no independent reference implementation to compare against.
- **No NEP26 user guides or calculator documentation published yet.** The NWAU calculators page (as of 7 May 2026) links only to the calculator files themselves. No separate methodology guide, technical note, or user manual specific to the 2026-27 calculators has been identified.
- **No checksums or hashes were previously captured for any downloaded artefact** — this inventory now closes that gap for the NWAU calculator artifacts (SHA-256 recorded above). PDF and XLSX price-weight downloads from the NEP and NEC resource pages still need checksums captured.
- **No separate standalone NEC price-weight table file was found**; price-weight material is published through the NEP determination and its appendix tables only.
...
- **The SAS `.7Z` archive has not been extracted.** Its internal contents (SAS programs, lookup tables, macros, etc.) are not enumerated here. Extraction and census of those contents is a downstream task.
- **Excel `.xlsb` files are raw, unmodified copies** of the IHACPA-published originals. No attempt has been made to unpack or audit the internal worksheet structure in this pass.
