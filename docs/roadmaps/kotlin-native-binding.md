...
This document compares the candidate integration strategies for the Kotlin/Native
...
- Provide a Kotlin/Native integration path for enterprise services, analytics
  jobs, and Kotlin-first consumers.
- Keep the shared Rust core as the only source of formula logic.
- Use the Kotlin/Native surface for transport, packaging, diagnostics, and host
  integration only.
...
| --- | --- | --- | --- |
| Arrow / Parquet file interop | Lowest binary packaging risk: no C ABI library distribution, no native symbol loading, and a contract that is easy to inspect, archive, and fixture-test. It works well across Kotlin/Native because both ecosystems already handle tabular and file-oriented data comfortably. | Slower than in-process calls and less ergonomic for fine-grained request/response flows. It adds file lifecycle management and is not ideal for synchronous interactive APIs. | Initial path for batch jobs, reproducible validation, and environments that need the safest cross-platform rollout. |
...
| C ABI | Lowest call overhead for in-process consumers and a natural fit for native Kotlin applications that must embed the engine directly. | Highest binary packaging risk: native libraries, platform classifiers, ABI compatibility, memory ownership, and cross-platform build distribution all become first-order concerns. C ABI reduces direct C ABI code but does not remove the native packaging problem. | Deferred option for latency-sensitive embedding only after the ABI and packaging rules are stable. |

## Decision

Start with **Arrow / Parquet file interop** as the initial Kotlin/Native strategy.
...
Defer **C ABI** until the ABI, memory-ownership model, and
binary packaging rules are stable enough to justify native embedding.

## Why Arrow / Parquet file interop comes first

- It minimizes binary packaging risk by avoiding C ABI or C ABI artifact
  distribution in the first native delivery.
...
- It aligns with Kotlin/Native tooling that already understands immutable
...
- Use Kotlin data classes where the supported runtime baseline allows it; otherwise
...
3. Promote C ABI / C ABI only after ABI stability, memory ownership, and binary
   packaging portability are proven.

## Validation rule

- Every Java or Kotlin example must validate against shared golden fixtures.
- A Kotlin/Native binding that duplicates formula constants, thresholds, or adjustment
  rules is out of scope.
- Release claims for Kotlin/Native package packaging should wait until the chosen
