...
PROC SQL;
	CREATE TABLE SUBACUTE00 AS
		SELECT t1.*,
			COALESCE(t2.RA2006,t3.RA2006,t1.&EST_REMOTENESS.,0) AS _pat_remoteness
		FROM &INPUT. t1
...
PROC SQL;
	CREATE TABLE &OUTPUT. AS
		...
		CASE 
			WHEN t2.care IS MISSING 
				OR (t4.ansnap_v3 IS NOT MISSING  AND COALESCE(t4.ansnap_pw_inlier,t4.ansnap_pw_outlier_perdiem) IS MISSING) THEN -2
	...
	AS _care_category FORMAT=SA_CARECAT.,
		CASE 
			...
			WHEN (t4.ansnap_inlier_lb IS MISSING) AND (t4.ansnap_pw_inlier IS NOT MISSING) THEN 1			
	...
	AS _pat_separation_category FORMAT=SA_SEPCAT.,
		CASE (CALCULATED _pat_separation_category)
			...
			WHEN 1 THEN t4.ansnap_pw_inlier
	...
	AS _w02 LABEL="Step 2: price weight after application of paediatric adjustment",
		(CALCULATED _w02) * (1 + t6.adj_indigenous + t5.adj_remoteness) AS GWAU13 FORMAT=20.4 LABEL = "2013-14 Gross Weighted Activity Unit",
	CASE 
...
AS error_code FORMAT = SA_ERROR. LABEL="Reason for 0 NWAU",
	t1._pat_private_flag * (1-t3.caretype_adj_privpat_serv) * (CALCULATED _w01) AS _adj_privpat_serv FORMAT=20.4 LABEL="Private Patient Service Deduction",
...
CASE 
	...
	ELSE MAX(0, CALCULATED GWAU13 - (CALCULATED _adj_privpat_serv) - (CALCULATED _adj_privpat_accomm))
END 
AS NWAU13 FORMAT=20.4 LABEL="2013-14 National Weighted Activity Unit"
FROM SUBACUTE01 t1
	LEFT JOIN CALCREF.NEP13_SA_PERDIEM_PRICE_WEIGHTS t2 ON (t1._care = t2.care)
	LEFT JOIN CALCREF.NEP13_SA_ADJ_PRIV_SERV t3 ON (t1._care = t3.care)
	LEFT JOIN CALCREF.NEP13_SA_SNAP_PRICE_WEIGHTS t4 ON (t1.&ANSNAP_V3. = t4.ansnap_v3)
		INNER JOIN CALCREF.NEP13_AC_SA_ADJ_REM t5 ON (t1._pat_remoteness = t5.pat_remoteness)
		INNER JOIN CALCREF.NEP13_SA_ADJ_IND t6 ON (t1._pat_ind_flag = t6.pat_ind_flag)
			LEFT JOIN CALCREF.NEP13_ADJ_PRIV_ACC t7 ON (t1.&STATE. = t7.state);
