...
%macro createindex; 
	...
	data _EDUDG_PRICE_WEIGHTS (index=(_UDG));
		set CALCREF.NEP18_EDUDG_PRICE_WEIGHTS (rename=(
		UDG=_UDG));
	run; 

	data _EDURG_PRICE_WEIGHTS (index=(&URG.)); 
		set CALCREF.NEP18_EDURG_PRICE_WEIGHTS (rename=(URG=&URG.)); 
	run; 

	data _ED_ADJ_AGE (index=(_pat_age_grp)); 
		set CALCREF.nep18_ed_adj_age ; 
	run; 
...
/*	CALCULATE GWAU18*/
	GWAU18= max(0,coalesce(_w01,0)*(1+adj_indigenous+adj_remoteness)*(1+adj_age)); 

/*	CALCULATE NWAU18*/
	If Error_Code > 0 then NWAU18=0; 
	else NWAU18= MAX(0, GWAU18);
/*=====================================================================*/
/*DEBUG MODE*/
/*=====================================================================*/
