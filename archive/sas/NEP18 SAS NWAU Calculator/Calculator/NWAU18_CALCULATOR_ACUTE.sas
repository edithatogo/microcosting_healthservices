/*
		+---------------------------------------------------------------+
		| Name:			NWAU18_CALCULATOR_ACUTE.sas						|
...
data _icu_paed_eligibility_list (index=(APCID&ID_YEAR.)); 
	set CALCREF.NEP18_icu_paed_eligibility_list; 
run;

data _AA_PRICE_WEIGHTS (index=(&DRG.));  
	set CALCREF.NEP18_AA_PRICE_WEIGHTS 
	(rename=(DRG=&DRG.));
run; 
...
data _ADJ_PRIV_ACC (index=(&state.));  
	set CALCREF.NEP18_ADJ_PRIV_ACC 
	(rename=(State=&state.));
run; 


data _AA_ADJ_SPA (index=(_pat_spa_category));
  SET CALCREF.NEP18_AA_ADJ_SPA;
RUN;
...
data _AA_SA_ADJ_REM (index=(_pat_remoteness));
  SET CALCREF.NEP18_AA_SA_ADJ_REM;
RUN;

data _AA_SA_ADJ_TREAT_REM (index=(_treat_remoteness));
  SET CALCREF.NEP18_AA_SA_ADJ_TREAT_REM;
RUN;

data _AA_ADJ_RT (index=(_pat_radiotherapy_flag));
  SET CALCREF.NEP18_AA_ADJ_RT;
RUN;

data _AA_ADJ_DS (index=(_pat_dialysis_flag));
  SET CALCREF.NEP18_AA_ADJ_DS;
RUN;	


data _null_ ;
	set CALCREF.NEP18_AA_ADJ_ICU; 
...
/*---------------------------------------------------------------------*/
	...
	%if &ICU_PAED_OPTION.=1 %then %do; 
		set _icu_paed_eligibility_list (keep=APCID&ID_YEAR. _est_eligible_icu_flag _est_eligible_paed_flag)  key=APCID&ID_YEAR./unique; 
...
/*	Patient Acute Flag */
	if int(&CARE_TYPE.) in (&INSCOPE_CARE.) /*or(INT(&CARE_TYPE.) = 7 AND SUM(0,&QLDAYS.) > 0)*/ then _pat_acute_flag=1; 
...
/*	MERGE DRG PRICE WEIGHTS*/
	set _AA_PRICE_WEIGHTS (drop=Description) key=&DRG./unique; 
	_drg_inscope_flag=1; 
	if _IORC_ ne 0 then do; 
		_error_ =0; 
...
/*	MERGE HOSPITAL REMOTENESS ADJUSTMENT*/
	...
	set _AA_SA_ADJ_TREAT_REM key=_treat_remoteness/unique; 
...
/*	MERGE RADIOTHEARPY ADJUSTMENT*/
	set _AA_ADJ_RT key=_pat_radiotherapy_flag/unique; 
...
/*	MERGE DIAYLSIS ADJUSTMENT*/
	set _AA_ADJ_DS key=_pat_dialysis_flag/unique; 
...
/*	CALCULATED PREDICTED AFTER INDIGENOUS AND REMOTE ADJUSTMENT W04*/
	_w04=_w03*(1+adj_indigenous+adj_remoteness+adj_radiotherapy+adj_dialysis)*(1+adj_treat_remoteness); 

/*	CALCULATED PREDICTED AFTER ICU ADJUSTMENT*/
	_adj_icu=coalesce(_pat_eligible_icu_hours*&ICU_RATE.,0);

/*	CALCULATED GWAU17*/
	GWAU18=max(0,sum(0,_w04,_adj_icu));
...
/*	CALCULATE NWAU18*/
	...
	else NWAU18= MAX(0, GWAU18 - _adj_privpat_serv -  _adj_privpat_accomm);
