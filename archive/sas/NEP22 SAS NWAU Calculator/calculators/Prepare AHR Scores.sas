...
data &output.(
keep= an100mdc_ra agegroup_rm DRG10_Type flag_emergency flag_ICUHours 
...
%if &func. = calc %then %do;
	&EP_ID._readm &EST_ID._readm temp_idx
...
sysecho "preparing AHR risk factors";
	set &input.( 
		%if &func.=calc %then %do; 
			where=(max(0,	&ahr_prefix.01_flag,&ahr_prefix.02_flag,&ahr_prefix.03_flag,
							&ahr_prefix.04_flag,&ahr_prefix.05_flag,&ahr_prefix.06_flag,
							&ahr_prefix.07_flag,&ahr_prefix.08_flag,&ahr_prefix.09_flag,
	...
	if _n_=1 then do;
		...
		dcl hash p_nep22(dataset: "CALCREF.nep22_aa_price_weights(rename=(	drg = &DRG.))");
		p_nep22.definekey("&DRG.");
		p_nep22.definedata("drg_inlier_lb");
		p_nep22.definedone();
	end;
