/************************************************
 ...
 Description : The Admitted Mental Health NWAU22 calculator. This file is called by the subacute TEMPLATE SAS program and should not be modified by
				the end	user.
...
/* Get Admitted price weights */
	...
	data _ADM_PRICE_WEIGHTS ( index = ( &AMHCC. ) ) ;
		...
		set CALCREF.NEP22_MH_ADM_PRICE_WEIGHTS	( rename = ( AMHCC = &AMHCC. ) ) ;
...
/* Get private patient accommodation adjustment, indexed by state */
	...
	data _ADJ_PRIV_ACC ( index = ( &state. ) ) ;
		...
		set CALCREF.nep22_ADJ_PRIV_ACC ( rename = ( state = &state. ) ) ;
...
/* Get national private patient service adjustment, indexed by ( state , amhcc ). */
	...
	data _ADJ_PRIV_SERV ( index = ( PPSA_ID = ( &amhcc. &state. ) ) ) ;
		...
		set CALCREF.nep22_MH_PPSA ( rename = ( amhccv1Code = &amhcc.
												state = &state.
...
/* Get specialist paediatric adjustment */
	...
	data _MH_ADJ_SPECPAED ( index = ( _pat_specpaed ) ) ;
		...
		set CALCREF.nep22_MH_ADJ_SPECPAED ;
...
/* Get specialist paediatric establishment */
	...
	data _paed_eligibility_list (index=(APCID&ID_YEAR.)); 
		...
		set CALCREF.nep22_icu_paed_eligibility_list; 
...
%if &CMTY_SSTREAM. = 1 %then %do ;
	...
	data _CMTY_PRICE_WEIGHTS ( index = ( &AMHCC. ) ) ;
		...
		set CALCREF.nep22_MH_CMTY_PRICE_WEIGHTS	( rename = ( AMHCC = &AMHCC. ) ) ;
...
/* If a patient doesn't have a length of stay, service contact counts or an AMHCC class then we can't price it so flag this patient with a ERROR_CODE. */

	%if &CMTY_SSTREAM. = 1 %then %do ;

	if missing ( &AMHCC. ) = 1 or missing ( &SC_PAT. ) = 1 or missing ( &SC_NOPAT. ) = 1  then error_code = 3 ;

	%end ;

	%if &ADM_SSTREAM. = 1 %then %do ;

	if missing ( _pat_los ) = 1 or missing ( &AMHCC. ) = 1 then error_code = 3 ;
...
/*	CALCULATE community substream gwau22 */

			&GWAU. = max( 0 , _cmty_fixed_pw + (&SC_PAT. * _cmty_sc_pat_pw) + (&SC_NOPAT. *_cmty_sc_nopat_pw) ) ;
