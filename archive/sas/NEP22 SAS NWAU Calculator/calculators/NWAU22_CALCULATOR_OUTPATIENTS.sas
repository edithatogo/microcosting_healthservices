/************************************************
 ...
 Description : The Non-admitted NWAU22 calculator. This file is called by the subacute TEMPLATE SAS program and should not be modified by the end
				user.
...
data _OP_PRICE_WEIGHTS ( index = ( &tier2_clinic. ) ) ;
	...
	set CALCREF.NEP22_OP_PRICE_WEIGHTS&shadow.
...
data _null_ ;
	...
	set CALCREF.NEP22_op_multi_prov_adj ;
...
data _nep22_hospital_ra2016 ( index = ( APCID&ID_YEAR. ) ) ;
...
/*	GET HOSPITAL REMOTENESS VALUE */
	...
	%if &EST_REMOTENESS_OPTION. = 1 %then %do ;
		...
		set _nep22_hospital_ra2016 ( keep = apcid&ID_YEAR. _hosp_ra_2016 ) key = apcid&ID_YEAR. /unique ;
