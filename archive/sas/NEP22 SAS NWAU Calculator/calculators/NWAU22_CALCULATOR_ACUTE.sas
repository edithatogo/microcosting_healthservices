/*
		+---------------------------------------------------------------+
		| Name:			NWAU22_CALCULATOR_ACUTE.sas						|
...
sysecho "reading in rates";
	set CALCREF.nep22_hospital_ra2016/*(rename=(APC_ID_&ID_YEAR.=APCID&ID_YEAR. ra2016=_hosp_ra_2016) keep=APC_ID_&ID_YEAR. ra2016)*/; 
run;

data _AA_PRICE_WEIGHTS (index=(&DRG.)); 
sysecho "reading in rates"; 
	set CALCREF.nep22_AA_PRICE_WEIGHTS
	(rename=(DRG = &DRG.));
run; 
...
sysecho "reading in rates";
	%if &PPSERVADJ. = 1 %then %do;
		set calcref.nep22_aa_adj_privpat_serv_nat
	%end;
	%else %if &PPSERVADJ. = 2 %then %do;
		set calcref.nep22_aa_adj_privpat_serv_jur
	%end;
	(rename=(drg = &drg.));
run;
...
/*	CALCULATED gwau22*/
	gwau22 = round ( max(0,sum(0,_w03,_adj_icu)) , 0.0000000001 ) ;
	if missing(_pat_los) then gwau22 = 0;
...
/*	CALCULATE nwau22*/
	If Error_Code > 0 then nwau22 = 0; 
	else nwau22 = round ( MAX(0, gwau22 - _adj_privpat_serv -  _adj_privpat_accomm) , 0.0000000001 ) ;
