			...
			set hacgrp.Table&T. end=eof;
				if _n_=1 then do;
					...
					if length(ICD_code) = 3 then do;
						if exception = 1 then 
							G2=cats('',G2,',"',ICD_code,'"');
						else G1=cats('',G1,',"',ICD_code,'"');
					end;
					else if specific = 1 then
						G3=cats('',G3,',"',ICD_code,'"');
				end;
				if eof then	do;
					call symput("TableG1",trim(G1));
					call symput("TableG2",trim(substr(G2,2)));
...
%set_var_list(work.Diagnosis_Map,hac031c02p01);
%set_var_list(work.Diagnosis_Map,hac031c02p02);
%set_var_list(work.Diagnosis_Map,hac031c02p03);
%set_var_list(work.Diagnosis_Map,hac031c04p01);
%set_var_list(work.Diagnosis_Map,hac031c04p03);
%set_var_list(work.Diagnosis_Map,hac031c06p01);
%set_var_list(work.Diagnosis_Map,hac031c08p01);
%set_var_list(work.Diagnosis_Map,hac031c10p01);
%set_var_list(work.Diagnosis_Map,hac031c10p04);
%set_var_list(work.Diagnosis_Map,hac031c10p05);
%set_var_list(work.Diagnosis_Map,hac031c14p02);
%macro set_var_list (input, condition); 
	data _null_;
	   length allvars $30000;
	   retain allvars ' ';
	 ...
	 run;
 %mend; 
%set_var_list(work.Diagnosis_Map,hac031c15p01); 
%set_var_list(work.Diagnosis_Map,hac031c15p02); 
%set_var_list(work.Diagnosis_Map,hac031c16p01); 
%set_var_list(work.Diagnosis_Map,hac031c16p02); 
%set_var_list(work.Diagnosis_Map,hac031c15p01);
%set_var_list(work.Diagnosis_Map,hac031c15p02);
%set_var_list(work.Diagnosis_Map,hac031c16p01);
%set_var_list(work.Diagnosis_Map,hac031c16p02);
...
/* Assume No Condition */
			...
			if ddx in (&hac031c02p01.)  then hac031c02p01=1*onset_mult*denom_main;
			if ddx in (&hac031c02p02.)  then hac031c02p02=1*onset_mult*denom_main;
			if ddx in (&hac031c02p03.)  then hac031c02p03=1*onset_mult*denom_main;

			if ddx in (&hac031c02p01.,&hac031c02p02.,&hac031c02p03.) and onset ='1' and onset_mult=1 and denom_main=1 then do;
		...
		hac031c02p01_Total=sum(0,hac031c02p01_Total,hac031c02p01*Fall_mult_final);
		hac031c02p02_Total=sum(0,hac031c02p02_Total,hac031c02p02*Fall_mult_final);
		hac031c02p03_Total=sum(0,hac031c02p03_Total,hac031c02p03*Fall_mult_final);
...
Note: DSS developed (METeOR identifier: 578317), not currently in NMDS.*/
		...
		hac031c04p01=0;
		hac031c04p01_mult_final=0;

		if ddx in (&hac031c04p01.) and onset in ('1') and onset_mult=1 and denom_main=1 then do;
			hac031c04p01=1;
			do j=1 to MAXSRGVAR;
				srgj=SRGARRAY[j];
				if srgj in ('1370601','1370602','1370603','9206000','9206100','9206200','9206300','9206400', '9220600', '1330600')
					then hac031c04p01_mult=1;
				else hac031c04p01_mult=0;
				hac031c04p01_mult_final=min(1,sum(0,hac031c04p01_mult,hac031c04p01_mult_final));
			end;
		end;
		hac031c04p01_total=sum(0,hac031c04p01_total,hac031c04p01*hac031c04p01_mult_final);
...
Enforce Sequencing (i.e. has to occure after diagnosis)*/
		hac031c04p03=0;
		hac031c04p03_mult_final=0;

		if ddx in (&hac031c04p03.) and onset in ('1') and onset_mult=1 and denom_main=1 then do;
			hac031c04p03=1;
			do j=i to MAXDDXVAR;
				ddxj = DDXARRAY[j];
					if ddxj in ('Y832')
						then hac031c04p03_mult=1;
					else hac031c04p03_mult=0;
				hac031c04p03_mult_final=min(1,sum(0,hac031c04p03_mult,hac031c04p03_mult_final));

			end;
		end;

		hac031c04p03_total=sum(hac031c04p03*hac031c04p03_mult_final,hac031c04p03_total);
...
AND any of: 13882-00, 13882-01, 13882-02, 92209-01, 92209-02*/
		...
		hac031c06p01=0;
		hac031c06p01_mult_final=0;

		if ddx in (&hac031c06p01.) and onset ='1' and onset_mult=1 and denom_main=1 then do;
			hac031c06p01=1;

			do j= 1 to MAXSRGVAR;
				srgj = SRGARRAY[j];
				if srgj in ('1388200','1388201','1388202','9220901', '9220902')
					then hac031c06p01_mult=1;
				else hac031c06p01_mult=0;
				hac031c06p01_mult_final=min(1,sum(0,hac031c06p01_mult,hac031c06p01_mult_final));

			end;

		end;

		hac031c06p01_total=sum(0,hac031c06p01*hac031c06p01_mult_final, hac031c06p01_total);
...
AND any procedure code of: 13100-00, 13100-02, 13100-04. Exclude episodes with EITHER N184 (any COF) or N185 (any COF)*/
		...
		hac031c08p01=0;
		hac031c08p01_mult_final=0;
		hac031c08p01_excl_final=0;

		if ddx in (&hac031c08p01.) and onset ='1' and onset_mult=1 and denom_main=1 then do;
			hac031c08p01=1;
			do j= 1 to MAXSRGVAR;
				srgj = SRGARRAY[j];
				if srgj in ('1310000','1310001','1310002','1310003','1310004')
					then hac031c08p01_mult=1;
				else hac031c08p01_mult=0;

				hac031c08p01_mult_final=min(1,sum(0,hac031c08p01_mult,hac031c08p01_mult_final));

			end;

			do j= 1 to MAXDDXVAR;
				ddxj = DDXARRAY[j];
				if ddxj in ('N184', 'N185') then hac031c08p01_excl_mult=1;
				else hac031c08p01_excl_mult=0;
				hac031c08p01_excl_final=min(1,sum(0,hac031c08p01_excl_mult,hac031c08p01_excl_final));
			end;
		end;

		hac031c08p01_total=sum(hac031c08p01*hac031c08p01_mult_final*(1-hac031c08p01_excl_final),hac031c08p01_total);
...
AND any external cause code of : X41, X42, Y11, Y12, Y13, Y14, X43, X44, Y45.0, Y47.0-Y47.9 (all with any COF)*/
		...
		hac031c10p01=0;
		hac031c10p01_mult_final=0;

		if ddx in (&hac031c10p01.) and onset in ('1') and onset_mult=1 and denom_main=1 then do;
			hac031c10p01=1;
			...
			do j= i to MAXDDXVAR;
				...
				if ddxj in ('X41','X42','Y11','Y12','Y13','Y14','X43','X44',
					...
					then hac031c10p01_mult=1;
					else hac031c10p01_mult=0;
				hac031c10p01_mult_final=min(1,sum(0,hac031c10p01_mult,hac031c10p01_mult_final));

			end;

		end;

		hac031c10p01_total=sum(0,hac031c10p01_total,hac031c10p01*hac031c10p01_mult_final);
...
AND any external cause code of : Y46, Y47, Y49, Y50*/
		...
		hac031c10p04p05_mult_final=0;

		if ddx in (&hac031c10p04.) then hac031c10p04=1*onset_mult*denom_main;
		if ddx in (&hac031c10p05.) then hac031c10p05=1*onset_mult*denom_main;

		if ddx in (&hac031c10p04., &hac031c10p05.) and onset in ('1') and onset_mult=1 and denom_main=1 then do;
			...
			do j= i to MAXDDXVAR;
				...
				if ddxj in ('Y460','Y461','Y462','Y463','Y464','Y465','Y466','Y467','Y468',
					...
					then hac031c10p04p05_mult=1;
					else hac031c10p04p05_mult=0;
				hac031c10p04p05_mult_final=min(1,sum(0,hac031c10p04p05_mult,hac031c10p04p05_mult_final));
			end;
		end;

		hac031c10p04_total=sum(0,hac031c10p04_total,hac031c10p04*hac031c10p04p05_mult_final);
		hac031c10p05_total=sum(0,hac031c10p05_total,hac031c10p05*hac031c10p04p05_mult_final);
...
3847300, 3847301, 3865400, 3865403, 9020200, 9020201, 9020202*/
		...
		hac031c14p02_mult_final=0;

		if ddx in (&hac031c14p02.) and onset in ('1') and onset_mult=1 and denom_main=1 then do;
			hac031c14p02=1;
			do j=1 to MAXSRGVAR;
				srgj=SRGARRAY[j];
				if srgj in ('3825600','3825601','3835000','3836800','3839000','3839001','3839002','3847000','3847001',
								'3847300','3847301','3865400','3865403','9020200','9020201','9020202')
					then hac031c14p02_mult=1;
				else hac031c14p02_mult=0;
				hac031c14p02_mult_final=min(1,sum(0,hac031c14p02_mult,hac031c14p02_mult_final));

			end;
		end;
		hac031c14p02_total=sum(0,hac031c14p02_total,hac031c14p02_mult_final);
...
Exclude if episode is ANY of the following:
    ...
    - Care type is 'Organ procurement-posthumous'  -  Care type = 9 "	*/
		...
		if ddx in (&hac031c15p01.) and i ne 1 then
			hac031c15p01=1;
		if ddx in (&hac031c15p02.) and i ne 1 then
			hac031c15p02=1;

		hac031c15p01_total=sum(0,hac031c15p01_total,hac031c15p01)*denom_15;
		hac031c15p02_total=sum(0,hac031c15p02_total,hac031c15p02)*denom_15;
...
Exclude if episode is ANY of the following:
   -  preterm infants, cases with injury to brachial plexus, and cases with osteogenesis imperfecta (Table A).
    ...
    - Care type is 'Organ procurement-posthumous'  -  Care type = 9 "			*/
		...
		if &care_type. in (7.1,7.2,7.3) and ddx in (&hac031c16p01.) then hac031c16p01=1;
		if &care_type. in (7.1,7.2,7.3) and ddx in (&hac031c16p02.) then hac031c16p02=1;

		hac031c16p01_total=sum(0,hac031c16p01_total,hac031c16p01)*denom_16;
		hac031c16p02_total=sum(0,hac031c16p02_total,hac031c16p02)*denom_16;
	...
	%if &output_mode. ne "counts" %then %do;
		...
		hac031c14p02_total=min(1,hac031c14p02_total);
		hac031c15p01_total=min(1,hac031c15p01_total);
	%end;
	...
	hac031c06p01_mult_final
	hac031c06p01_mult

	hac031c10p01_mult_final
	hac031c10p01_mult
	hac031c10p04p05_mult_final
	hac031c10p04p05_mult

	hac031c08p01_mult_final
	hac031c08p01_excl_final
	hac031c08p01_mult
	hac031c08p01_excl_mult

	hac031c04p03_mult_final
	hac031c04p03_mult

	hac031c04p01_mult_final
	hac031c04p01_mult

	hac031c14p02_mult_final
	hac031c14p02_mult
