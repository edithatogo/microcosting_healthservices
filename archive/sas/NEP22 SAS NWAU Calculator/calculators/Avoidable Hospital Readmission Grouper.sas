...
*
		...
		| Description:	2021-20 Readmissions Grouper         		                |
		| Version:		1.1															|
...
libname AHRGRP "&LOCATION." access=readonly;
%global keep_vars;
%macro keepv;
%let KEEP_VARS= temp_idx &STATE. &admmode. onset: &indstat. ;
%if &func.=calc %then %let keep_vars=&keep_vars. &icu_hours. &icu_other. ; ;
%mend keepv;
%keepv;
...
%let readm_list=AHR010c01p01 AHR010c01p02 AHR010c01p03 AHR010c02p01 AHR010c02p02 AHR010c02p03 AHR010c02p04 AHR010c02p05 AHR010c02p06 AHR010c02p07 AHR010c02p08 AHR010c02p09 AHR010c02p10 AHR010c03p01 AHR010c03p02 AHR010c03p03 AHR010c03p04 AHR010c03p05 AHR010c03p06 AHR010c04p01 AHR010c04p02 AHR010c05p01 AHR010c06p01 AHR010c07p01 AHR010c08p01 AHR010c08p02 AHR010c09p01 AHR010c10p01 AHR010c10p02 AHR010c10p03 AHR010c10p04 AHR010c11p01 AHR010c12p01;
%let readm_list_c=AHR010c01p01_flag,AHR010c01p02_flag,AHR010c01p03_flag,AHR010c02p01_flag,AHR010c02p02_flag,AHR010c02p03_flag,AHR010c02p04_flag,AHR010c02p05_flag,AHR010c02p06_flag,AHR010c02p07_flag,AHR010c02p08_flag,AHR010c02p09_flag,AHR010c02p10_flag,AHR010c03p01_flag,
				AHR010c03p02_flag,AHR010c03p03_flag,AHR010c03p04_flag,AHR010c03p05_flag,AHR010c03p06_flag,AHR010c04p01_flag,AHR010c04p02_flag,AHR010c05p01_flag,AHR010c06p01_flag,AHR010c07p01_flag,AHR010c08p01_flag,AHR010c08p02_flag,AHR010c09p01_flag,AHR010c10p01_flag,
				AHR010c10p02_flag,AHR010c10p03_flag,AHR010c10p04_flag,AHR010c11p01_flag,AHR010c12p01_flag;
...
%let keep_in=&EP_ID. &EST_ID. &BIRTH_DATE.
				...
				&KEEP_VARS.;
%let keep_out=&UNIQUE_ID. &EP_ID. &EST_ID. year &STATE. &HOSP_RA. &PEERGROUP_CODE. &pat_postcode. &pat_sa2.
...
proc sql noprint;
title "AHR010c02p06v08";
select ddx into :AHR010c02p06v08 separated by '","' from local.ahr_map_08 where AHR010c02p06 = 1;
title "AHR010c02p06v09";
select ddx into :AHR010c02p06v09 separated by '","' from local.ahr_map_09 where AHR010c02p06 = 1;
title "AHR010c02p06v10";
select ddx into :AHR010c02p06v10 separated by '","' from local.ahr_map_10 where AHR010c02p06 = 1;
title "AHR010c02p06v11";
select ddx into :AHR010c02p06v11 separated by '","' from local.ahr_map_11 where AHR010c02p06 = 1;
title "AHR010c02p06v12";
select ddx into :AHR010c02p06v12 separated by '","' from local.ahr_map_12 where AHR010c02p06 = 1;
title "AHR010c08p02v08";
select ddx into :AHR010c08p02v08 separated by '","'  from local.ahr_map_08 where AHR010c08p02 = 1;
title "AHR010c08p02v09";
select ddx into :AHR010c08p02v09 separated by '","'  from local.ahr_map_09 where AHR010c08p02 = 1;
title "AHR010c08p02v10";
select ddx into :AHR010c08p02v10 separated by '","'  from local.ahr_map_10 where AHR010c08p02 = 1;
title "AHR010c08p02v11";
select ddx into :AHR010c08p02v11 separated by '","'  from local.ahr_map_11 where AHR010c08p02 = 1;
title "AHR010c08p02v12";
select ddx into :AHR010c08p02v12 separated by '","'  from local.ahr_map_12 where AHR010c08p02 = 1;
quit;
				...
				if onsetarray[i]="2" and i >= 2 then
					do;
						if scan("&ICD10AM_EDITION.",year + 1) = "08" then do;
							if ddxarray[i] in ("&AHR010c02p06v08.") then _AHR010c02p06=1;
							if ddxarray[i] in ("&AHR010c08p02v08.") then _AHR010c08p02=1;
						end;
						else if scan("&ICD10AM_EDITION.",year + 1) = "09" then do;
							if ddxarray[i] in ("&AHR010c02p06v09.") then _AHR010c02p06=1;
							if ddxarray[i] in ("&AHR010c08p02v09.") then _AHR010c08p02=1;
						end;
							...
							if ddxarray[i] in ("&AHR010c02p06v11.") then _AHR010c02p06=1;
							if ddxarray[i] in ("&AHR010c08p02v11.") then _AHR010c08p02=1;
						end;
						else if scan("&ICD10AM_EDITION.",year + 1) = "12" then do;
							if ddxarray[i] in ("&AHR010c02p06v12.") then _AHR010c02p06=1;
							if ddxarray[i] in ("&AHR010c08p02v12.") then _AHR010c08p02=1;
						end;
					end;
...
proc sort data=&output.1 out=&output.1; 
	sysecho "joining years and flagging diagnoses";
	by &state. &UNIQUE_ID. &ADM_DATE. &SEP_DATE. &EP_ID.;
run;

data badpin_list_a(keep=&unique_id. badpin_readm);
sysecho "creating list of bad &unique_id.";
	...
	if &UNIQUE_ID. = lag_&UNIQUE_ID. and (&SEX. ne lag_&SEX. or &BIRTH_DATE. ne lag_&BIRTH_DATE.) then badpin_readm=1; else delete;
...
sysecho "filtering out concurrent episodes and episodes with bad &unique_id.";
	...
	if _n_=1 then do;
		...
		bp_all.definedata("badpin_readm");
		bp_all.definedone();
	end;
...
run;
proc sort data=&output.3; by &state. &UNIQUE_ID. &ADM_DATE. &SEP_DATE. &EP_ID.;run;
...
sysecho "determine time between separations";
	...
	if &UNIQUE_ID. = lag_&UNIQUE_ID. and &ADM_DATE. >= lag_&SEP_DATE. and not missing(&BIRTH_DATE.) and readm_trim=0 and lag_index_trim=0 and badpin_readm = 0 then do;
		readm=1;
		...
		* AHMAC intervals ;
		...
		if AHR010c03p01 and surgicalIndex and readm_time <= 28 then AHR010c03p01_flag = 1;
		if AHR010c03p02 and surgicalIndex and readm_time <= 28 then AHR010c03p02_flag = 1;
		if AHR010c03p03 and surgicalIndex and readm_time <= 28 then AHR010c03p03_flag = 1;
		if AHR010c03p04 and surgicalIndex and readm_time <= 28 then AHR010c03p04_flag = 1;
		if AHR010c03p05 and surgicalIndex and readm_time <= 14 then AHR010c03p05_flag = 1;
		if AHR010c03p06 and surgicalIndex and readm_time <= 28 then AHR010c03p06_flag = 1;
		...
		if AHR010c12p01 and not(drugIndex) and readm_time <= 7 then AHR010c12p01_flag = 1;
...
DATA &OUTPUT.5 (keep=index_flag sameyear &EP_ID._readm &EST_ID._readm year_readm ahr010c: &AHR_PREFIX.01_flag &AHR_PREFIX.02_flag &AHR_PREFIX.03_flag &AHR_PREFIX.04_flag &AHR_PREFIX.05_flag &AHR_PREFIX.06_flag &AHR_PREFIX.07_flag &AHR_PREFIX.08_flag &AHR_PREFIX.09_flag &AHR_PREFIX.10_flag &AHR_PREFIX.11_flag &AHR_PREFIX.12_flag /*GWAU_readm riskAdjustment_readm NWAU_readm*/ badpin_index);
sysecho "attribute readmission details";
	SET &OUTPUT.4(firstobs=2 rename=(readm_flag=index_flag
					badpin_readm=badpin_index &EP_ID.=&EP_ID._readm  &EST_ID.=&EST_ID._readm year=year_readm) drop=_AHR010c02p06 _AHR010c08p02 );
...
DATA &OUTPUT.(drop=badpin_index badpin_readm i);
sysecho "join readmission info to index episode";
	MERGE &OUTPUT.4(drop=sameyear &readm_list. 
AHR010c01p01_flag AHR010c01p02_flag AHR010c01p03_flag AHR010c02p01_flag 
							AHR010c02p02_flag AHR010c02p03_flag AHR010c02p04_flag AHR010c02p05_flag AHR010c02p06_flag AHR010c02p07_flag AHR010c02p08_flag AHR010c02p09_flag 
            				AHR010c02p10_flag AHR010c03p01_flag AHR010c03p02_flag AHR010c03p03_flag AHR010c03p04_flag AHR010c03p05_flag AHR010c03p06_flag AHR010c04p01_flag 
							AHR010c04p02_flag AHR010c05p01_flag AHR010c06p01_flag AHR010c07p01_flag AHR010c08p01_flag AHR010c08p02_flag AHR010c09p01_flag AHR010c10p01_flag
							AHR010c10p02_flag AHR010c10p03_flag AHR010c10p04_flag AHR010c11p01_flag AHR010c12p01_flag 
	...
	array readmx[*] c:;
	if eof then do;
		do i=1 to dim(readmx);
			if missing(readmx[i]) then readmx[i] = 0;
		end;
	end;
run;
