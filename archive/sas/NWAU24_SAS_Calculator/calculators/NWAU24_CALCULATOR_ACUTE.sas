/*
		+-------------------------------------------------------------------+
		| File:			NWAU24_CALCULATOR_ACUTE.sas							|
		| Name:			2024-25 Acute Admitted Patients						|
		|				National Weighted Activity Unit Calculator			|
		| Version:		1.0													|
		| Author:		Jada Ching											|
		| Modified: 	Christopher Deeb 31/10/2023							|	
		|				Pricing Section										|
		|				Independent Health and Aged Care Pricing Authority	|
		| Description:	The Acute Admitted NWAU24 calculator. This file is 	|
...
sysecho "reading in rates";
	set CALCREF.nep24_hospital_ra2021/*(rename=(APC_ID_&ID_YEAR.=APCID&ID_YEAR. ra2021=_hosp_ra_2021) keep=APC_ID_&ID_YEAR. ra2021)*/; 
run;

data _AA_PRICE_WEIGHTS (index=(&DRG.)); 
sysecho "reading in rates"; 
	set CALCREF.nep24_AA_PRICE_WEIGHTS
	(rename=(DRG = &DRG.));
run; 
...
sysecho "reading in rates";
	%if &PPSERVADJ. = 1 %then %do;
		set calcref.nep24_aa_adj_privpat_serv_nat
	%end;
	%else %if &PPSERVADJ. = 2 %then %do;
		set calcref.nep24_aa_adj_privpat_serv_jur
	%end;
	(rename=(drg = &drg.));
run;
...
sysecho "reading in rates";
	set CALCREF.nep24_AA_ADJ_COVID;
run;

data _null_ ;
sysecho "reading in rates";
	set CALCREF.nep24_AA_ADJ_ICU; 
...
/*	CALCULATED gwau23*/
	gwau24 = round ( max(0,sum(0,_w04,_adj_icu)) , 0.0000000001 ) ;
	if missing(_pat_los) then gwau24 = 0;
...
/*	CALCULATE nwau24*/
	If Error_Code > 0 then nwau24 = 0; 
	else nwau24 = round ( MAX(0, gwau24 - _adj_privpat_serv -  _adj_privpat_accomm) , 0.0000000001 ) ;
