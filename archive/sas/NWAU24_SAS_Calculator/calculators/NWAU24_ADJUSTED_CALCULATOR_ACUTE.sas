/*
		+-------------------------------------------------------------------------------+
		| Name:			NWAU24_ADJUSTED_CALCULATOR_ACUTE.sas							|			
...
%macro update_nwau;
	...
	%if &hac_adjustment. ne 0 or &ahr_adjustment. ne 0 %then %do;
		...
		data &calculator_output.;
			set &output.(	drop=nwau24);
			nwau24				= round ( nwau24_temp , 0.0000000001 ) ;
...
/*=====================================================================*/
			drop urgadm
				%if &DEBUG_MODE. = 0 %then %do;	
					_:	state_:	adj_: 	drg_: nwau24_temp
					%if &hac_adjustment. ne 0 %then %do;
