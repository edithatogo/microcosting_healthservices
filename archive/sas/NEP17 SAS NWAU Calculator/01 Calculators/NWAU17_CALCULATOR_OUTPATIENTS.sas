/*
		+---------------------------------------------------------------+
		| Name:			NWAU16_CALCULATOR_OUTPATIENTS.sas				|
		| Description:	2016-17 Outpatients								|
...
data _OP_PRICE_WEIGHTS (index=(&tier2_clinic.)); 
	set CALCREF.NEP17_OP_PRICE_WEIGHTS 
	(rename=(
...
data _null_ ;
	set CALCREF.nep17_op_multi_prov_adj; 
...
/*	CALCULATED GWAU17 */
	if &pat_multiprov_flag. =1 then GWAU17=sum(clinic_pw*(1+adj_indigenous)*(1+&adj_multiprov.),0); 
	else GWAU17=max(0,sum(clinic_pw*(1+adj_indigenous),0)); 
