/*
		+---------------------------------------------------------------+
		| Name:			NWAU16_CALCULATOR_ACUTE.sas						|
...
%global RADIOTHERAPY; /* SBGW 13NOV2018 11:41AM - corrected type in variable name*/
%global DIALYSIS; 
...
data _icu_paed_eligibility_list (index=(APCID&ID_YEAR.)); 
	set CALCREF.nep17_icu_paed_eligibility_list; 
run;

data _AA_PRICE_WEIGHTS (index=(&DRG.));  
	set CALCREF.NEP17_AA_PRICE_WEIGHTS 
	(rename=(DRG=&DRG.));
run; 
...
/*	CALCULATED GWAU17*/
	GWAU17=max(0,sum(0,_w04,_adj_icu));
...
/*	CALCULATE NWAU17*/
	If Error_Code > 0 then NWAU17=0; 
	else NWAU17= MAX(0, GWAU17 - _adj_privpat_serv -  _adj_privpat_accomm);
