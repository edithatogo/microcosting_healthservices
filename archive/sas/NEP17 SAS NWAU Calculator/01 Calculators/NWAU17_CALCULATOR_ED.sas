...
%macro createindex; 
	...
	data _EDUDG_PRICE_WEIGHTS (index=(_UDG));
		set CALCREF.NEP17_EDUDG_PRICE_WEIGHTS (rename=(
		UDG=_UDG));
	run; 

	data _EDURG_PRICE_WEIGHTS (index=(&URG.)); 
		set CALCREF.NEP17_EDURG_PRICE_WEIGHTS (rename=(URG=&URG.)); 
	run; 

	data _ED_ADJ_AGE (index=(_pat_age_grp)); 
		set CALCREF.nep17_ed_adj_age ; 
	run; 
%mend; 
...
/*	CALCULATE GWAU17*/
	GWAU17= max(0,coalesce(_w01,0)*(1+adj_indigenous)*(1+adj_age)); 

/*	CALCULATE NWAU17*/
	If Error_Code > 0 then NWAU17=0; 
	else NWAU17= MAX(0, GWAU17);
/*=====================================================================*/
/*DEBUG MODE*/
/*=====================================================================*/
