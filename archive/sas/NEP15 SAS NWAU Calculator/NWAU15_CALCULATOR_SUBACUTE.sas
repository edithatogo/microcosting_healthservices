/*
		+---------------------------------------------------------------+
		| Name:			NWAU15_CALCULATOR_SUBACUTE.sas						|
		| Description:	2015-16 SubAcute Admitted Patients					|
...
data NEP15_SA_SNAP_PRICE_WEIGHTS (index=(&ANSNAP_V3.));  
	set CALCREF.NEP15_SA_SNAP_PRICE_WEIGHTS 
	(rename=(ansnap=&ANSNAP_V3.));
run; 

data NEP15_SA_PAED_PRICE_WEIGHTS (index=(_care));  
	set CALCREF.NEP15_SA_PAED_PRICE_WEIGHTS 
	(rename=(caretype=_care));
run; 
data NEP15_SA_ADJ_PRIV_SERV (index=(_care));  
	set CALCREF.NEP15_SA_ADJ_PRIV_SERV 
	(rename=(caretype=_care));
run; 
...
/*	MERGE SNAP PRICE WEIGHTS*/
	set NEP15_SA_SNAP_PRICE_WEIGHTS (drop=Description ) key=&ANSNAP_V3./unique; 
...
/*	MERGE PAEDIATRIC PRICE WEIGHTS*/
	...
	set  NEP15_SA_PAED_PRICE_WEIGHTS key=_care/unique; 
...
/*	MERGE PRIVATE PATIENT SERVICE ADJUSTMENT*/
	set NEP15_SA_ADJ_PRIV_SERV key=_care/unique; 
...
/*	MERGE PRIVATE PATIENT ACCOMODATION ADJUSTMENT*/
	set CALCREF.NEP15_ADJ_PRIV_ACC key=state/unique; 
...
/*---------------------------------------------------------------------*/
	...
	else if missing(&ANSNAP_V3.)=1 and _pat_age_years  gt 17 then Error_Code=3; 
...
/*	SEPARATION CATEGORY*/
	if &ANSNAP_V3. in ('3-112') then _pat_separation_category=2; 
	else if ansnap_samedaylist_flag=1 then _pat_separation_category=1; 
	else if _pat_eligible_paed_flag=1 then _pat_separation_category=0; 
	else if _pat_sameday_flag=1 and (ansnap_samedaylist_flag=1 or ansnap_type='Overnight Palliative Care') then _pat_separation_category=1; 
...
/*	CALCULATED GWAU15*/
	GWAU15=max(0,_w01*(1+adj_indigenous+adj_remoteness)); 
...
proc sql; 
drop table NEP15_SA_SNAP_PRICE_WEIGHTS; 
drop table POSTCODE_TO_RA2011; 
drop table ASGS_TO_RA2011; 
drop table SLA_TO_RA2011; 
drop table NEP15_SA_PAED_PRICE_WEIGHTS; 
drop table NEP15_SA_ADJ_PRIV_SERV; 
quit; 
