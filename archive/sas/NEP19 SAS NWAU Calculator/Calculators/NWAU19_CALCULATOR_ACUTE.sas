/*
		+---------------------------------------------------------------+
		| Name:			NWAU19_CALCULATOR_ACUTE.sas						|
...
data _icu_paed_eligibility_list (index=(APCID&ID_YEAR.)); 
	set CALCREF.NEP19_icu_paed_eligibility_list; 
run;
%end;

data _AA_PRICE_WEIGHTS (index=(&DRG.));  
	set CALCREF.NEP19_AA_PRICE_WEIGHTS 
	(rename=(DRG=&DRG.));
run; 
...
/*	CALCULATED GWAU19*/
	GWAU19=max(0,sum(0,_w04,_adj_icu));
