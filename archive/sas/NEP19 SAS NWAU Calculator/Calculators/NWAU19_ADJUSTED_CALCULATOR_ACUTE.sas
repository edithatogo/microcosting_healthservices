/*
		+-------------------------------------------------------------------------------+
		| Name:			NWAU19_ADJUSTED_CALCULATOR_ACUTE.sas							|			
...
%let debug_mode=1;
%include "&location.\NWAU19_CALCULATOR_ACUTE.sas";
	...
	%if &risk_adjustment. ne 0 %then %do;
		...
		data &calculator_output.;
			set inter_qwau(rename=(sex=&sex.
								   ...
								   riskAdjustment=rAdjustment) drop=nwau19);
			Error_Code=eCode;
			hacflag=hflag;
			hacgroup=hgroup;
			complexity=cPoint;
