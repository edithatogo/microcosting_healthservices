/*
		+---------------------------------------------------------------+
		| Name:			NWAU14_CALCULATOR_OUTPATIENTS.sas				|
		| Description:	2014-15 Non-Admitted Patients					|
...
PROC SQL;
	CREATE TABLE &OUTPUT. AS
		SELECT t1.*,
			...
			t2.clinic_pw * (1 + t3.adj_indigenous) FORMAT=20.4 AS GWAU14 FORMAT=20.4 LABEL = "2014-15 Gross Weighted Activity Unit",
			CASE WHEN (CALCULATED error_code > 0) THEN 0
			ELSE (CALCULATED GWAU14) END
			AS NWAU14 FORMAT = 20.4 LABEL="2014-15 National Weighted Activity Unit"
		FROM OUTPATIENTS00 t1
			LEFT JOIN CALCREF.NEP14_OP_PRICE_WEIGTHS t2 ON (t1.&CLINIC. = t2.tier2_clinic)
				INNER JOIN CALCREF.NEP14_AC_ED_OP_ADJ_IND t3 ON (t1.pat_ind_flag = t3.pat_ind_flag);
QUIT;
