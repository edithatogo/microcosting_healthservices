...
/*This will be executed if radiotherapy_option = 2*/
%macro import;
	DATA ACUTE00;
		SET &INPUT.;
...
/*This will be executed if radiotherapy_option = 1*/
%macro get_radio_flags;
	DATA ACUTE00;
		SET &INPUT.;
		ARRAY procedure &PROC_PREFIX.:;
		DO OVER procedure;
			IF COMPRESS(procedure, '/- ') IN 
				(
				'1500000','1500300','1510000','1510300','1522400','1523900','1525400',
				'1526900','1560000','1560001','1560002','1560003','1560004','1530300',
				'1530400','1531100','1531200','1531900','1532000','9076400','9076401',
				'1532700','1532701','1532702','1532703','1532704','1532705','1532706',
				'1532707','1533800','1536000','1533900','1501200','9076600','1600300',
				'1600900','1601200','1601500','1601800','9096000',

				'1534200','1535100','9076500','9076501','9076502','9076503','9076504',
				'1550000','1550300','1550600','1550601','1550602','1550900','1555000',
				'1551800','1552100','1552400','1552401','1552700','1553000','1553300',
				'1553600','1553601','1553602','1553900','1554100','1555600','1555601'
					) THEN
				_pat_radiotherapy_flag = 1;
		END;
...
DATA _NULL_;
	IF &RADIOTHERAPY_OPTION. = 2 THEN
		CALL EXECUTE('%import');
	ELSE CALL EXECUTE('%get_radio_flags');
RUN;
...
%macro join_hosp_flags;
	...
	PROC SQL;
		CREATE TABLE ACUTE01 AS
			...
			FROM ACUTE00 t1 
				LEFT JOIN CALCREF.NEP14_ICU_PAED_ELIGIBILITY_LIST t2 ON (t1.&APCID. = t2.APCID&ID_YEAR.);
		DROP TABLE ACUTE00;
	QUIT;
...
PROC SQL;
	CREATE TABLE ACUTE01A AS
		SELECT t1.*,
			COALESCE(t2.RA2011,t3.RA2011,&EST_REMOTENESS.,0) AS _pat_remoteness
		FROM ACUTE01 t1
...
PROC SQL;
	...
	AS _w02 FORMAT=20.4 LABEL="Step 2: price weight after application of paediatric adjustments"
		FROM ACUTE02 t1
			LEFT JOIN CALCREF.NEP14_ACUTE_PRICE_WEIGHTS t2 ON(t1.&DRG. = t2.DRG);
	DROP TABLE ACUTE02;
QUIT;

/*Define global variable icu_rate*/
DATA _NULL_;
	SET CALCREF.NEP14_ACUTE_ADJ_ICU;
...
PROC SQL;
	CREATE TABLE ACUTE04 AS
		SELECT t1.*,
			...
			(CALCULATED _w03) * (1 + t3.adj_indigenous + t4.adj_remoteness + t5.adj_radiotherapy) AS _w04 FORMAT=20.4 LABEL="Step 4: price weight after application of Indigenous, remote and radiotherapy adjustments",
			t1._pat_eligible_icu_hours * &ICU_RATE. AS _adj_icu FORMAT=20.4 LABEL = "ICU Adjustment",
			(CALCULATED _w04) + (CALCULATED _adj_icu) AS GWAU14 FORMAT=20.4 LABEL = "2014-15 Gross Weighted Activity Unit"
		FROM ACUTE03 t1
			INNER JOIN CALCREF.NEP14_ACUTE_ADJ_SPA t2 ON (t1._pat_spa_category = t2.pat_spa_category)
			INNER JOIN CALCREF.NEP14_AC_ED_OP_ADJ_IND t3 ON (t1._pat_ind_flag = t3.pat_ind_flag)
			INNER JOIN CALCREF.NEP14_AC_SA_ADJ_REM t4 ON (t1._pat_remoteness = t4.pat_remoteness)
			INNER JOIN CALCREF.NEP14_ACUTE_ADJ_RT t5 ON (t1._pat_radiotherapy_flag = t5.pat_radiotherapy_flag);
	DROP TABLE ACUTE03;
QUIT;
...
PROC SQL;
	CREATE TABLE &OUTPUT. AS
		SELECT t1.*,
			t1._pat_private_flag * t1._drg_adj_privpat_serv * (t1._w01 + t1._adj_icu) AS _adj_privpat_serv FORMAT=20.4 LABEL="Private Patient Service Deduction",
	...
	AS error_code FORMAT=AC_ERROR. LABEL="Reason for 0 NWAU",
		CASE 
			...
			ELSE MAX(0, t1.GWAU14 - (CALCULATED _adj_privpat_serv) - (CALCULATED _adj_privpat_accomm))
		END 
	AS NWAU14 FORMAT = 20.4 LABEL="2014-15 National Weighted Activity Unit"
		FROM ACUTE04 t1
			LEFT JOIN CALCREF.NEP14_ADJ_PRIV_ACC t2 ON (t1.&STATE. = t2.State);
	DROP TABLE ACUTE04;
QUIT;
