/*
		+---------------------------------------------------------------+
		| Name:			NWAU14_CALCULATOR_ED.sas						|
		| Description:	2014-15 Emergency Department Patients			|
...
PROC SQL;
	CREATE TABLE &OUTPUT. AS
		...
		COALESCE(CALCULATED _w01,0) * (1 + t4.adj_indigenous) AS NWAU14 FORMAT=10.4 LABEL="2014-15 National Weighted Activity Unit"
		FROM ED00 t1

		LEFT JOIN CALCREF.NEP14_EDUDG_PRICE_WEIGHTS t2 ON (t1.udg = t2.udg) /* !!! THIS CHANGE WAS DONE TO ACCOMADATE ES DATASET !!!;
/*        	LEFT JOIN CALCREF.NEP14_EDUDG_PRICE_WEIGHTS t2 ON (t1._udg = t2.udg) */

		LEFT JOIN CALCREF.NEP14_EDURG_PRICE_WEIGHTS t3 ON (t1.&URG. = t3.urg)
		INNER JOIN CALCREF.NEP14_AC_ED_OP_ADJ_IND t4 ON (t1._pat_ind_flag = t4.pat_ind_flag);
	DROP TABLE ED00;
QUIT;
