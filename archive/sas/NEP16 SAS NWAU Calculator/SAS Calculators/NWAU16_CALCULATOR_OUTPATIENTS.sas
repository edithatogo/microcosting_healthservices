...
data _OP_PRICE_WEIGHTS (index=(&tier2_clinic.)); 
	set CALCREF.NEP16_OP_PRICE_WEIGHTS 
	(rename=(
...
data _null_ ;
	set CALCREF.nep16_op_multi_prov_adj; 
...
/*	CALCULATED GWAU16 */
	if &pat_multiprov_flag. =1 then GWAU16=sum(clinic_pw*(1+adj_indigenous)*(1+&adj_multiprov.),0); 
	else GWAU16=max(0,sum(clinic_pw*(1+adj_indigenous),0)); 
