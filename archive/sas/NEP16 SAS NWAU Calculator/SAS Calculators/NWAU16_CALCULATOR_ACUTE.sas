...
data _icu_paed_eligibility_list (index=(APCID&ID_YEAR.)); 
	set CALCREF.nep16_icu_paed_eligibility_list; 
run;

data _AA_PRICE_WEIGHTS (index=(&DRG.));  
	set CALCREF.NEP16_AA_PRICE_WEIGHTS 
	(rename=(DRG=&DRG.));
run; 
...
data _AA_ADJ_RT (index=(_pat_radiotherapy_flag));
  SET CALCREF.NEP16_AA_ADJ_RT;
RUN;

data _AA_ADJ_DS (index=(_pat_dialysis_flag));
  SET CALCREF.NEP16_AA_ADJ_DS;
RUN;	


data _null_ ;
	set CALCREF.NEP16_AA_ADJ_ICU; 
...
/*	CALCULATED GWAU16*/
	GWAU16=max(0,sum(0,_w04,_adj_icu));
