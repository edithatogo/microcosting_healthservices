/*
		+-------------------------------------------------------------------+
		| File:			NWAU23_CALCULATOR_MH.sas							|
		| Name:			2023-24 Admitted Mental Health Patients				|
		...
		| Description: 	The Admitted Mental Health NWAU23 calculator. This 	|
		|				file is called by the mental health TEMPLATE SAS 	|
...
/* Get Admitted price weights */
	...
	data _ADM_PRICE_WEIGHTS ( index = ( &AMHCC. ) ) ;
		...
		set CALCREF.NEP23_MH_ADM_PRICE_WEIGHTS	( rename = ( AMHCC = &AMHCC. ) ) ;
...
/* Get private patient accommodation adjustment, indexed by state */
	...
	data _ADJ_PRIV_ACC ( index = ( &state. ) ) ;
		...
		set CALCREF.nep23_ADJ_PRIV_ACC ( rename = ( state = &state. ) ) ;
...
/* Get national private patient service adjustment, indexed by ( state , amhcc ). */
	...
	data _ADJ_PRIV_SERV ( index = ( PPSA_ID = ( &amhcc. &state. ) ) ) ;
		...
		set CALCREF.nep23_MH_PPSA ( rename = ( amhccv1Code = &amhcc.
												state = &state.
...
/* Get specialist paediatric adjustment */
	...
	data _MH_ADJ_SPECPAED ( index = ( _pat_specpaed ) ) ;
		...
		set CALCREF.nep23_MH_ADJ_SPECPAED ;
...
%if &CMTY_SSTREAM. = 1 %then %do ;
	...
	data _CMTY_PRICE_WEIGHTS ( index = ( &AMHCC. ) ) ;
		...
		set CALCREF.nep23_MH_CMTY_PRICE_WEIGHTS	( rename = ( AMHCC = &AMHCC. ) ) ;
...
/*	CALCULATE community substream gwau23 */

			&GWAU. = max( 0 , _cmty_fixed_pw + (&SC_PAT_PUB. * _cmty_sc_pat_pw) + (&SC_NOPAT_PUB. *_cmty_sc_nopat_pw) ) ;
