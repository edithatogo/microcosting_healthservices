/*
		+-------------------------------------------------------------------+
		| File:			NWAU23_CALCULATOR_SUBACUTE.sas						|
		| Name:			2023-24 Admitted Subacute Patients					|
		|				National Weighted Activity Unit Calculator			|
		| Version:		1.0													|
		| Author:		Jada Ching											|
		| Modified: 	Jessica Skoog 02/11/2022							|	
		|				Pricing Section										|
		|				Independent Health and Aged Care Pricing Authority	|
		| Description : The Admitted Subacute NWAU23 calculator. This 		|
...
data _SA_SNAP_PRICE_WEIGHTS ( index = ( &ANSNAP. ) ) ;
	...
	set CALCREF.NEP23_SA_SNAP_PRICE_WEIGHTS&shadow.
...
data _SA_ADJ_PRIV_SERV_STATE ( index = ( ppsa_st_key = ( &STATE. _care ) ) ) ;  
	...
	set CALCREF.NEP23_SA_ADJ_PRIV_SERV_STATE&shadow. ( rename = ( caretype = _care state = &STATE. ) ) ;
...
/* Carry out SNAPv4 checks if not using the shadow model. */
	if &SHADOW. = 0 then do;

		if _pat_age_years ge 0 and _pat_age_years le 17 and &ANSNAP. not in ( '4F01' ,'4F02' ,'4F03' ,'4F04' ,'4F05' ,'4O01' ,'4ES5' , '4G01' , '4G02' , '4G03' , '4G04' , '4P01', '4EL1' ) then  error_Code = 1 ;
...
/* If using the SNAPv5 shadow weights, carry out an end-class level check. */
	else if &SHADOW. = 1 then do;
