...
data &output.;
	...
	%if &readmissions. ne 0 %then %do;
		...
		riskAdjustment_readm = w01_readm*max(of readm_adj:);
	...
	nwau21_temp = max(0, nwau21 - riskAdjustment_HAC - riskAdjustment_readm);
	...
	if max(0, nomatch_age, nomatch_mdc, nomatch_icu, nomatch_cc, 
				...
				nomatch_flag_instrument_use,nomatch_flag) ne 0 and hacflag = 1 then do;
