/************************************************
 ...
 Description : The Admitted Subacute NWAU21 calculator. This file is called by the subacute TEMPLATE SAS program and should not be modified by the end
				user.
...
data _SA_SNAP_PRICE_WEIGHTS ( index = ( &ANSNAP_V4. ) ) ;
	...
	set CALCREF.NEP21_SA_SNAP_PRICE_WEIGHTS
...
data _SA_ADJ_PRIV_SERV_STATE ( index = ( ppsa_st_key = ( &STATE. _care ) ) ) ;  
	...
	set CALCREF.NEP21_SA_ADJ_PRIV_SERV_STATE ( rename = ( caretype = _care state = &STATE. ) ) ;
...
data _AA_SA_ADJ_RT ( index = ( _pat_radiotherapy_flag ) ) ;
	...
	set CALCREF.NEP21_AA_SA_ADJ_RT ;
...
data _AA_SA_ADJ_DS ( index = ( _pat_dialysis_flag ) ) ;
	...
	set CALCREF.NEP21_AA_SA_ADJ_DS ;
...
/* Error for missing snap class, date or state information.  */
	...
	else if missing ( &ANSNAP_V4. ) or missing ( &ADM_DATE. ) or missing ( &SEP_DATE. ) or missing ( &state. ) then error_Code = 3 ;
...
/* Apply the relevant adjustments to _w01 based on the patient and hospital characteristics of this separation. */
	...
	&GWAU. = max ( 0 , _w01 * ( 1 + adj_indigenous + adj_remoteness + adj_radiotherapy + adj_dialysis ) * ( 1 + adj_treat_remoteness ) ) ; 
...
/* If there is no error then obtain the NWAU by subtracting the private patient adjustments from the GWAU. If the number obtained is negative then
	...
	else &NWAU. = max ( 0 , &GWAU. - _adj_privpat_serv - _adj_privpat_accomm ) ;
