/*
		+---------------------------------------------------------------+
		| Name:			NWAU21_CALCULATOR_ACUTE.sas						|
...
sysecho "reading in rates";
	set CALCREF.NEP21_hospital_readm(rename=(APC_ID_&ID_YEAR.=APCID&ID_YEAR. ra2016=_hosp_ra_2016) keep=APC_ID_&ID_YEAR. ra2016); 
run;

data _AA_PRICE_WEIGHTS (index=(&DRG.)); 
sysecho "reading in rates"; 
	set CALCREF.nep21_AA_PRICE_WEIGHTS&shadow.
	(rename=(DRG = &DRG.));
run; 
...
sysecho "reading in rates";
	%if &PPSERVADJ. = 1 %then %do;
		set calcref.nep21_aa_adj_privpat_serv_nat&shadow.
	%end;
	%else %if &PPSERVADJ. = 2 %then %do;
		set calcref.nep21_aa_adj_privpat_serv_jur&shadow.
	%end;
	(rename=(drg = &drg.));
run;
...
sysecho "reading in rates";
	set CALCREF.nep21_ADJ_PRIV_ACC
	(rename=(State = &state.));
run;

data _AA_ADJ_SPA (index=(_pat_spa_category));
	set CALCREF.NEP21_AA_ADJ_SPA&shadow.;
run;

data _AA_SA_ED_OP_ADJ_IND (index=(_pat_ind_flag));
sysecho "reading in rates";
	set CALCREF.nep21_AA_SA_NA_ED_ADJ_IND&shadow.;
run;

data _AA_SA_NA_ADJ_REM (index=(_pat_remoteness));
sysecho "reading in rates";
	set CALCREF.nep21_AA_SA_NA_ADJ_REM&shadow.;
run;

data _AA_SA_NA_ADJ_TREAT_REM (index=(_treat_remoteness));
sysecho "reading in rates";
	set CALCREF.nep21_AA_SA_NA_ADJ_TREAT_REM&shadow.;
run;

data _AA_SA_ADJ_RT (index=(_pat_radiotherapy_flag));
sysecho "reading in rates";
	set CALCREF.nep21_AA_SA_ADJ_RT&shadow.;
run;

data _AA_SA_ADJ_DS (index=(_pat_dialysis_flag));
sysecho "reading in rates";
	set CALCREF.nep21_AA_SA_ADJ_DS&shadow.;
run;

data _null_ ;
sysecho "reading in rates";
	set CALCREF.nep21_AA_ADJ_ICU&shadow.; 
...
/*=====================================================================*/
/*STEP 4: Calculate Patient Remotness Area*/
/*=====================================================================*/
/*Variables Created: 
	A04: _pat_remoteness
/*---------------------------------------------------------------------*/
...
/*	CALCULATED gwau21*/
	gwau21 = max(0,sum(0,_w03,_adj_icu));
	if missing(_pat_los) then gwau21 = 0;
...
/*	CALCULATE nwau21*/
	If Error_Code > 0 then nwau21 = 0; 
	else nwau21 = MAX(0, gwau21 - _adj_privpat_serv -  _adj_privpat_accomm);
