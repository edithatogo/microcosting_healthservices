/************************************************
 ...
 Description : The Non-admitted NWAU21 calculator. This file is called by the subacute TEMPLATE SAS program and should not be modified by the end
				user.
...
/* These macros indicate the year of the GWAU and NAWU you are calculating.*/
...
data _OP_PRICE_WEIGHTS ( index = ( &tier2_clinic. ) ) ;
	...
	set CALCREF.NEP21_OP_PRICE_WEIGHTS
...
data _AA_SA_NA_ED_ADJ_IND ( index = ( _pat_ind_flag ) ) ;
	...
	set CALCREF.NEP21_AA_SA_NA_ED_ADJ_IND ;
...
data _null_ ;
	...
	set CALCREF.NEP21_op_multi_prov_adj ;
...
data _AA_SA_NA_ADJ_REM ( index = ( _pat_remoteness ) ) ;
	...
	set CALCREF.NEP21_AA_SA_NA_ADJ_REM ;
...
data _AA_SA_NA_ADJ_TREAT_REM ( index = ( _treat_remoteness ) ) ;
	...
	set CALCREF.NEP21_AA_SA_NA_ADJ_TREAT_REM ;
...
data _nep21_hospital_ra2016 ( index = ( APCID&ID_YEAR. ) ) ;
...
/*	GET HOSPITAL REMOTENESS VALUE */
	...
	%if &EST_REMOTENESS_OPTION. = 1 %then %do ;
		...
		set _nep21_hospital_ra2016 ( keep = apcid&ID_YEAR. _hosp_ra_2016 ) key = apcid&ID_YEAR. /unique ;
...
/*For patient level datasets*/
	...
	/*	CALCULATE GWAU */
			...
			if _pat_eligible_paed_flag = 1 and &pat_multiprov_flag. = 1 then
				...
				&GWAU. = sum ( clinic_pw * tier2_adj_paed * ( 1 + adj_indigenous + adj_remoteness + &adj_multiprov. ) * ( 1 + adj_treat_remoteness ) , 0 ) ;

			else if _pat_eligible_paed_flag ne 1 and &pat_multiprov_flag. = 1 then

				&GWAU. = sum ( clinic_pw * ( 1 + adj_indigenous + adj_remoteness + &adj_multiprov. ) * ( 1 + adj_treat_remoteness ) , 0 ) ;
 
			else if _pat_eligible_paed_flag = 1 and &pat_multiprov_flag. ne 1 then

				&GWAU. = sum ( clinic_pw * tier2_adj_paed * ( 1 + adj_indigenous + adj_remoteness ) * ( 1 + adj_treat_remoteness ) , 0 ) ;

			else &GWAU. = max ( 0 , sum ( clinic_pw * ( 1 + adj_indigenous + adj_remoteness ) * ( 1 + adj_treat_remoteness ) , 0 ) ) ;
...
/*For aggregate level datasets*/
	...
	/*	CALCULATE GWAU */
			...
			if &pat_multiprov_flag. = 1 then 
				...
				&GWAU. = sum ( clinic_pw * ( 1 + &adj_multiprov. ) * ( 1 + adj_treat_remoteness ) , 0 ) * ( &GROUP_EVENT_COUNT. + &INDIV_EVENT_COUNT. ) ;

			else &GWAU. = sum ( clinic_pw * ( 1 + adj_treat_remoteness ) , 0 ) * ( &GROUP_EVENT_COUNT. + &INDIV_EVENT_COUNT. ) ;
