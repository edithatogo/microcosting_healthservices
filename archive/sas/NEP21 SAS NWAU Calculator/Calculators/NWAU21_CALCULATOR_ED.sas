/************************************************
 ...
 Description : The Emergency Department and Emergency Service NWAU21 calculator. This file is called by the subacute TEMPLATE SAS program and should not be 
...
/* If classification option < 3 then get the UDG price weights */
	...
	%if &CLASSIFICATION_OPTION. < 3 %then %do ;
		...
		data _EDUDG_PRICE_WEIGHTS ( index = ( &UDG. ) ) ;
			...
			set CALCREF.nep21_EDUDG_PRICE_WEIGHTS ( rename = ( UDG = &UDG. ) ) ;
...
/* If classification option = 3 then get AECC price weights */
	...
	%else %if &CLASSIFICATION_OPTION. = 3 %then %do ;
		...
		data _EDAECC_PRICE_WEIGHTS ( index = ( &AECC. ) rename = ( AECC = &AECC. ) ) ;
			...
			set CALCREF.nep21_EDAECC_PRICE_WEIGHTS ;
...
/* Get ESTID-to-remoteness map (used for the hospital remoteness adjustment) */
	...
	data _ESTID_to_ra2016 ( index = ( apcid&ID_YEAR. ) ) ;
...
/* Get remoteness (0-4) to hospital remoteness adjustment (%) map. */
	...
	data _ED_ADJ_TREAT_REM ( index = ( _treat_remoteness ) ) ;
		...
		set CALCREF.nep21_ED_ADJ_TREAT_REM ;
...
/* If &EST_REMOTENESS_OPTION. = 1 then &EST_REMOTENESS. hasn't been declared yet so we set it equal to _hosp_ra_2016 */
		...
		set _ESTID_to_ra2016 key = apcid&ID_YEAR. /unique ;

		%let EST_REMOTENESS = _hosp_ra_2016 ;

/* If the hospital's Establishment ID isn't listed in _hosp_ra_2016 then set the remoteness to zero. No error will be thrown and no 
...
/* PATIENT REMOTENESS BY POSTCODE */
		...
		set _POSTCODE_TO_RA2016 ( rename = ( RA2016 = PC_RA2016 ) ) key = &pat_postcode. /unique ;

/* If the patient's postcode is not listed in _POSTCODE_TO_RA2016 then we set postcode remoteness to MISSING */
...
/* If the patient's ASGS SA2 is not listed in _SA2_TO_RA2016 then we set SA2 remoteness to MISSING */
...
/* To find patient remoteness use in order, ASGS, postcode then as a proxy use hospital remoteness */
		...
		_pat_remoteness = coalesce ( SA2_RA2016 , PC_RA2016 , _treat_remoteness ) ;
...
/*	CALCULATE BASE PRICE WEIGHT AND GWAU */
	...
	%if &CLASSIFICATION_OPTION. < 3 %then %do ;
		...
		&GWAU. = coalesce ( _w01 * ( 1 + adj_treat_remoteness ) , 0 ) ;
	...
	%else %do ;
		...
		&GWAU. = coalesce ( _w01 * ( 1 + adj_indigenous + adj_remoteness ) * ( 1 + adj_treat_remoteness ) , 0 ) ;
