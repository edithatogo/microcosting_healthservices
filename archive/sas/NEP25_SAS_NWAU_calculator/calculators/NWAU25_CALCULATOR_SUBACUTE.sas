/*
		+-------------------------------------------------------------------+
		| File:			NWAU24_CALCULATOR_SUBACUTE.sas						|
		| Name:			2024-25 Admitted Subacute Patients					|
		|				National Weighted Activity Unit Calculator			|
		| Version:		1.0													|
		| Author:		Jada Ching											|
		| Modified: 	Christopher Deeb 01/11/2023							|	
		|				Pricing Section										|
		|				Independent Health and Aged Care Pricing Authority	|
		| Description : The Admitted Subacute NWAU24 calculator. This 		|
		|				file is called by the subacute TEMPLATE SAS 		|
...
/* Record the type of error to throw for each entry. */
	...
	value SA_ERROR
		...
		4 = "Same day flag and LOS greater than 1 in conflict";
...
data _SA_SNAP_PRICE_WEIGHTS ( index = ( &ANSNAP. ) ) ;
	...
	set CALCREF.NEP25_SA_SNAP_PRICE_WEIGHTS
...
data _SA_ADJ_PRIV_SERV_STATE ( index = ( ppsa_st_key = ( &STATE. _care ) ) ) ;  
	...
	set CALCREF.NEP25_SA_ADJ_PRIV_SERV_STATE ( rename = ( caretype = _care state = &STATE. ) ) ;
...
data _APCID_to_ra2021 ( index = ( &ESTID. ) ) ;
 
	set CALCREF.nep25_hospital_ra2021 ;
...
/* If &EST_REMOTENESS_OPTION. = 1 then &EST_REMOTENESS. hasn't been declared yet so we set it equal to _hosp_ra_2021 */
		...
		set _APCID_to_ra2021 key = &ESTID. /unique ;
...
/* PATIENT REMOTENESS BY ASGS */
	...
	_pat_remoteness = coalesce ( SA2_RA2021 , PAT_RA2021 , _treat_remoteness ) ;
...
/*	Patient LOS */
	&leave. = coalesce (&leave., 0);
	 pat_los = max ( 1 , SUM( DATDIF ( &ADM_DATE. , &SEP_DATE. , 'ACT/ACT' ) , - &leave. ) ) ;
...
/* If the admission or separation dates are missing or if the admission date is before the separation date then we set length of stay to MISSING. */
	 ...
	 if missing ( &ADM_DATE. ) or missing ( &SEP_DATE. ) then pat_los = . ;

	 else if ( &ADM_DATE. > &SEP_DATE. ) then pat_los = . ;
...
/*Variables Created: 
	...
	S26: caretype_adj_privpat_serv_state
...
/* Code to carry out SNAPv4 checks is commented out as no shadow is being used this year. */
	/* if &SHADOW. = 0 then do; */

		/* if _pat_age_years ge 0 and _pat_age_years le 17 and &ANSNAP. not in ( '4F01' ,'4F02' ,'4F03' ,'4F04' ,'4F05' ,'4O01' ,'4ES5' , '4G01' , '4G02' , '4G03' , '4G04' , '4P01' ) then  error_Code = 1 ; */
...
/* Carry out an end-class level check for SNAP v5. */
	

		if _pat_age_years ge 0 and _pat_age_years le 17 and &ANSNAP. not in ( '5F01' ,'5F02' ,'5F03' ,'5F04' ,'5F05' ,'5O01' , '5G01' , '5G02' , '5G03' , '5G04' , '5P01', '5ES4', '5EL1' ) then error_Code = 1;
		...
		/*Error if patient has same-day ANSNAP flag but Length of Stay is greater than 1*/ 
		else if &ANSNAP. in ('5J01','5O01','5K01','5P01','5L01','5M01') and (pat_los > 1 or (not (ansnap_samedaylist_flag))) then error_code = 4;
...
/*	SEPARATION CATEGORY */
	...
	else if pat_los lt ansnap_inlier_lb  then _pat_separation_category = 2 ;

	else if pat_los le ansnap_inlier_ub then _pat_separation_category = 3 ;

	else if pat_los gt ansnap_inlier_ub then _pat_separation_category = 4 ;
...
/* If this is a short-stay outlier then assign LOS times the short-stay outlier per diem for the relevant snap class. */
	...
	else if _pat_separation_category = 2 then _w01 = pat_los * coalesce ( ansnap_pw_sso_perdiem , 0 ) ;
...
/* If this is a LSO then assign the inlier flat rate PLUS the LSO per diem for every day about the inlier upper bound for this snap class. */
															...
															( pat_los - ansnap_inlier_ub ) * COALESCE( ansnap_pw_lso_perdiem , 0 ) ;
...
/* Accommodation Deduction : The accommodation deduction is determined by the state/territory of the separation and by whether or not this is a
	...
	_adj_privpat_accomm = _pat_private_flag * ( _pat_sameday_flag * state_adj_privpat_accomm_sd + 
																			...
																			( 1 - _pat_sameday_flag ) * pat_los * state_adj_privpat_accomm_on ) ;
