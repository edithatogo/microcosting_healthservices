/*
		+-------------------------------------------------------------------------------+
		| Name:			NWAU25_ADJUSTED_CALCULATOR_ACUTE.sas							|			
		| Description:	2025-26 Acute Admitted Patients									|
...
%macro temp; %mend temp;
	...
	%if &HAC_adjustment. = 1 %then %do;
		...
		* Choose options for Mental Health Cohort;
		%let MentalH_option = 1 ;	*Option 0: does not create cohorts;
		...
		%include "&LOCATION.\Hospital Acquired Complication Grouper 032.sas";
	
		%let grouper_prefix = 032; 
		data &output.;
			set &output.;
			&HAC_prefix.c01_flag= min(1,sum(of HAC&grouper_prefix.c01:,0));
			&HAC_prefix.c02_flag= min(1,sum(of HAC&grouper_prefix.c02:,0));
			&HAC_prefix.c03_flag= min(1,sum(of HAC&grouper_prefix.c03:,0));
			&HAC_prefix.c04_flag= min(1,sum(of HAC&grouper_prefix.c04:,0));
			&HAC_prefix.c05_flag= min(1,sum(of HAC&grouper_prefix.c05:,0));
			&HAC_prefix.c06_flag= min(1,sum(of HAC&grouper_prefix.c06:,0));
			&HAC_prefix.c07_flag= min(1,sum(of HAC&grouper_prefix.c07:,0));
			&HAC_prefix.c08_flag= min(1,sum(of HAC&grouper_prefix.c08:,0));
			&HAC_prefix.c09_flag= min(1,sum(of HAC&grouper_prefix.c09:,0));
			&HAC_prefix.c10_flag= min(1,sum(of HAC&grouper_prefix.c10:,0));
			&HAC_prefix.c11_flag= min(1,sum(of HAC&grouper_prefix.c11:,0));
			&HAC_prefix.c12_flag= min(1,sum(of HAC&grouper_prefix.c12:,0));
			&HAC_prefix.c13_flag= min(1,sum(of HAC&grouper_prefix.c13:,0));
			&HAC_prefix.c14_flag= min(1,sum(of HAC&grouper_prefix.c14:,0));
			&HAC_prefix.c15_flag= min(1,sum(of HAC&grouper_prefix.c15:,0));
			&HAC_prefix.c15p02_flag = min(1, HAC&grouper_prefix.c15p02);
			&HAC_prefix.c16_flag= min(1,sum(of HAC&grouper_prefix.c16:,0));
...
%macro temp; %mend temp;
	%if &AHR_adjustment. = 1 %then %do;
		...
		%let HOSPITAL_LIST = CALCREF.nep25_hospital_readm;
		%let HOSPITAL_ID = apcid&ID_YEAR.;
				
		%let PEERGROUP_CODE = peergroup_code_old;
		%let LHN_NAME = lhn_name;
		%let INSCOPE_FLAG = nep25_abf_flag;
		%let HOSP_RA = ra2021;
		
		libname local "&LOCATION.";
		%include "&LOCATION.\Avoidable Hospital Readmission Grouper 030.sas";
...
%macro update_nwau;
	...
	%if &hac_adjustment. ne 0 or &ahr_adjustment. ne 0 %then %do;
		...
		data &calculator_output.;
			set &output.(	drop=nwau25);
			nwau25				= round ( nwau25_temp , 0.0000000001 ) ;
...
/*=====================================================================*/
			...
			%if &DEBUG_MODE. = 0 %then %do;	
				drop _:	state_:	adj_: 	drg_: nwau25_temp
					%if &hac_adjustment. ne 0 %then %do;
