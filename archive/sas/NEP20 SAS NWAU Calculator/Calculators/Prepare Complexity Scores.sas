...
/******************************************************************************************************
	...
	These are:
		...
		-	drg10 type (Medical or Intervention)
		- 	MDC	
...
data drg10_MASTERLIST_RA (index=(_drg10)); 
	set CALCREF.drg10_MASTERLIST (rename=(	drg10 = _drg10 
											mdc10 = an100mdc_ra));
run; 

data &output. (drop = _drg10);
	...
	/*Create drg10 type*/
	_drg10 = &drg.;
	set drg10_MASTERLIST_RA (keep = _drg10 
									drg10_type 
									an100mdc_ra) 
							key = _drg10 / unique;
...
*HAC15 risk flag;
	if srg in: ( "9047002", "9047004", "9046800", "9046801", "9046802"
				,"9046803", "9046804", "9046805", "9046900", "9046901"
				,"9046806")
			then instrument_use_flag = 1;
	/*else instrument_use_flag = 0;*/
	end;
	...
	*More HAC15 risk factors;	
			else if DDX in: 		('Z3551','Z356' )																	then primiparity_flag = 1;																									 
			else if DDX in: 		('O328'	,'O640'	)  																	then PPOP_flag = 1;																						 
			else if DDX in: 		('O68'	)																			then foetal_distress_flag = 1;																								 
	end;
